...
echo $newdate;?>">
		...
		<div class="col-md-7">
			...
			<table border="1" class="table table-striped">
				...
				<tbody>
					<?php 
					$no = 1;
					$data = mysqli_query($conn,"select * from tbahliwaris");
					while($d=mysqli_fetch_array($data)){
						?>
						<tr>
							<td><?php echo $no++; ?></td>
							<td><?php echo $d['no_reg']; ?></td>
							<td><?php echo $d['waris_istri']; ?></td>
							<td><?php echo $d['waris_suami']; ?></td>
							<td><?php echo $d['nama_ahli_waris']; ?></td>
	...
	<script>
		...
		var myChart = new Chart(ctx, {
			type: 'bar',
			data: {
				labels: ["Diterima", "Ditolak","Perbaiki","Diproses"],
				datasets: [{
					label: '',
					data: [
					<?php 
                    ...
                    $tgl_akhir = $_POST['tgl_akhir'];
                        ...
                        $diterima = mysqli_query($conn,"SELECT * FROM tbahliwaris 
					...
					<?php 
                        ...
                        $ditolak = mysqli_query($conn,"SELECT * FROM tbahliwaris 
				   ...
				   $perbaiki = mysqli_query($conn,"SELECT * FROM tbahliwaris 
				   WHERE tanggal BETWEEN '$tgl_awal' AND '$tgl_akhir' AND  status_pengajuan='Perbaiki'");
				   echo mysqli_num_rows($perbaiki);
				   ...
				   $diproses = mysqli_query($conn,"SELECT * FROM tbahliwaris 
				   WHERE tanggal BETWEEN '$tgl_awal' AND '$tgl_akhir' AND  status_pengajuan='Diproses'");
